...
findBranch=re.compile(r'<a href="(.*?)" target="_blank" rel="noopener noreferrer" data-za-detail-view-element_name="Title">')
findFile=re.compile(r'<h1 class="Post-Title">(.*?)</h1>', re.S)
# findPages=re.compile(r'<li data-page="(\d+)" class="ui-pager">.*?</li>',re.S)

headers = {
"user-agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.75 Safari/537.36 Edg/86.0.622.38"
}
baseUrl='https://www.zhihu.com/people/one-seventh/posts?page='
# 页面数量为动态
...
def askUrl(baseUrl,i):
    ...
    for BranchUrl in branchUrls:
        ...
        #pdf文件的保存位置
        fileUrl = 'D:\\desktop\\zhihu\\' + ''.join(finalTitle) + '.pdf'
